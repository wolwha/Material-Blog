...
export default async function Page() {
  ...
  return (
    <>
      <div className="h-full rounded-[20px] bg-(--color-custom-white) justify-center items-start flex">
        <Container posts={fetchResult.data} />
        {hasUser ? <EditButton /> : null}
      </div>
      {/* 모바일 플로팅 메뉴 */}
      <div className="sm:hidden sticky bottom-6.25 justify-center flex items-center w-full z-20">
        <FloatingButton/>
      </div>
    </>
  );
